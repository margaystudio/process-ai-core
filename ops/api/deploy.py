...
Extiende el deploy del template con:
- soporte de [service].allow_unauthenticated (igual al patrón de margay-hub)
- `[secrets]` → `gcloud run deploy --update-secrets=...`: si la clave es una ruta absoluta
  (p. ej. `/secrets/etl_ssh/id_rsa`), Cloud Run monta el secret como archivo en esa ruta;
  si la clave es un identificador sin slashes, se inyecta como variable de entorno.
"""
...
def exists(cmd: list[str]) -> bool:
    """True si el comando devuelve exit code 0 (útil para jobs)."""
...
def kv_csv(d: Dict[str, Any]) -> str:
    ...
    Soporta comas dentro de valores (ej. JSON arrays) usando un delimitador
    alternativo con sintaxis gcloud ^<delim>^... .
    """
    if not d:
        return ""
    ...
    # Si ninguna key/value contiene coma, el formato CSV estándar es suficiente.
    if all("," not in p for p in pairs):
        return csv

    # Elegir delimitador seguro que no aparezca en las key/value.
    for delim in ("@", "|", "~", "#", ";", ":"):
        if all(delim not in p for p in pairs):
            return f"^{delim}^" + delim.join(pairs)

    raise SystemExit(
        "No se pudo serializar env vars para gcloud: no hay delimitador seguro "
        "disponible para valores con comas."
    )


def env_for_cloud_run_service(env: Dict[str, Any]) -> Dict[str, Any]:
    """Cloud Run Services reservan ciertas variables (p. ej. PORT)."""
    reserved = {"PORT"}
    return {k: v for k, v in env.items() if k not in reserved}


def pick_latest_release(releases_dir: Path, image_name: str) -> Path:
    """Elige el último archivo de release por mtime."""
    files = sorted(
        releases_dir.glob(f"{image_name}_*.txt"), key=lambda p: p.stat().st_mtime, reverse=True
    )
    if not files:
        raise SystemExit(
            f"No release files found in {releases_dir}. Run: python ops/api/release.py --env <env>"
        )
    return files[0]


def resolve_immutable(cfg_path: Path, image_name: str, image_override: str, release_file: str) -> str:
    """Decide qué digest deployar (promote)."""
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(description="Deploy/Promote (API): promote digest to Cloud Run.")
