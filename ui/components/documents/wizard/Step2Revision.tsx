...
// TODO(wire): reemplazar con el título y cuerpo reales devueltos por la API de generación
        ...
        <div className="flex items-center gap-2.5 border-b border-line-soft p-[16px_22px]">
          ...
          {!editing ? (
            <button
              ...
              className="inline-flex h-[34px] items-center gap-1.5 rounded-[9px] border border-line-input bg-surface px-3.5 text-[13px] font-bold text-ink-800 transition-colors hover:bg-surface-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-action"
            >
              <WizardIcon
            ...
            <>
              <button
                ...
                className="h-[34px] rounded-[9px] border border-line-input bg-surface px-[13px] text-[13px] font-semibold text-ink-500 transition-colors hover:bg-surface-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-action"
              >
                Cancelar
              </button>
              {/* TODO(wire): "Listo" debería guardar cambios vía API (PATCH versión DRAFT) */}
              <button
                type="button"
                onClick={() => setEditing(false)}
                className="h-[34px] rounded-[9px] bg-ink-800 px-3.5 text-[13px] font-bold text-white transition-colors hover:bg-ink-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-action"
              >
                Listo
              </button>
            </>
        ...
        <div className="p-[22px_26px]">
          {!editing ? (
            <article className="text-sm leading-7 text-ink-700">
              {body.split("\n\n").map((para, i) => {
                ...
                if (isHeading) {
                  ...
                  return (
                    <div key={i}>
                      <h3 className="mt-4 text-[13px] font-extrabold text-indigo">
                        {head}
                      </h3>
                      <p className="mt-1.5">{rest.join(" ")}</p>
                    </div>
                  );
                }
                return (
            ...
            <textarea
              ...
              className="min-h-[320px] w-full resize-y rounded-[10px] border border-line-input p-3.5 text-sm leading-7 text-ink-700 outline-none focus:border-indigo focus:ring-[3px] focus:ring-indigo-tint"
