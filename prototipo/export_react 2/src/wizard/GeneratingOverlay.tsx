// GeneratingOverlay — full-screen "La IA está armando el borrador" state,
// shown between Step 1 and Step 2 while the draft is generated.
import React from "react";
import { GEN_STEPS, Icon } from "./data";
    ...
    <div className="flex h-full flex-col items-center justify-center bg-surface-app px-6">
      <span className="relative mb-[18px] grid h-14 w-14 place-items-center">
        ...
        <span className="relative grid h-14 w-14 place-items-center rounded-[16px] bg-indigo text-white">
          <Icon d="M5 3l1.2 3.2L9 7.5 5.8 8.8 5 12 4.2 8.8 1 7.5l3.2-1.3zM17 11l1 2.6L21 15l-2.6 1L17 19l-1-2.6L13 15l2.6-1z" size={26} />
        </span>
      </span>
      <div className="mb-1 text-xl font-extrabold text-ink-900">La IA está armando el borrador</div>
      <div className="mb-[26px] text-center text-[13px] text-ink-400">Un momento — esto lo hace el sistema con las evidencias que cargaste.</div>
      <div className="flex w-full max-w-[420px] flex-col gap-[13px]">
        {GEN_STEPS.map((label, i) => {
          const done = i < current, active = i === current;
          return (
            <div key={label} className="flex items-center gap-3">
              <span className={"grid h-[26px] w-[26px] flex-shrink-0 place-items-center rounded-full " + (done ? "bg-green" : active ? "bg-indigo-tint" : "bg-surface-track")}>
